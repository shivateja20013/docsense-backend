...
async def fn_get_chat_sessions_by_user(username: str):
    # try:
        with sqlite3.connect(DATABASE_PATH) as conn:
            cursor = conn.cursor()
            cursor.execute('''
                SELECT id, title, systemPrompt, temperature, createdAt, lastAccessedAt
                FROM ChatSession
                WHERE username = ?
            ''', (username,))
            ...
            if result:
                response_data = [{"id": row[0], "title": row[1], "systemPrompt": row[2], "temperature": row[3],
                                  "createdAt": row[4], "lastAccessedAt": row[5]} for row in result]
                return Response(content=json.dumps(response_data), status_code=200)
            else:
                return Response(content=json.dumps("No chat sessions found for this chatbot."), status_code=404)
